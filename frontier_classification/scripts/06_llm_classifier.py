...
Batching: the free tier's daily request quota is small (as low as 20/day for
some models), so poems are batched into a handful of prompts (one prompt asks
about several poems at once and returns a JSON array) instead of one request
per poem. 40 poems in batches of 10 costs 4 requests, not 40.
...
Usage:
    ...
    python 06_llm_classifier.py --mode gold_recall --sample 40 --batch-size 40  # single request
"""
import argparse
import json
import os
...
def classify_batch(client, model_name, batch):
    """batch: list of (title, body). Returns list of dicts (or None per-item on failure)."""
    poems_block = "\n\n".join(
...
def main():
    ...
    ap.add_argument("--batch-size", type=int, default=10,
                     help="poems per API request (higher = fewer requests = less quota used, "
                          "but bigger risk of one bad response losing the whole batch)")
    ...
    # Resume support: skip poems already classified in a previous (possibly
    # quota-cut-short) run.
    ...
    todo = [r for r in rows if (r[0], r[1]) not in already_done]
    print(f"{len(todo)} poems to classify in batches of {args.batch_size} "
          f"({(len(todo) + args.batch_size - 1)//args.batch_size} API request(s))")
    ...
    with open(out_path, "a", encoding="utf-8") as f:
        for start in range(0, len(todo), args.batch_size):
            ...
            batch_input = [(title, body) for _, title, body, _ in chunk]
            results = classify_batch(client, args.model, batch_input)

            for (author, title, _, true_label), result in zip(chunk, results):
                if result is None:
                ...
                if true_label is not None:
                    ...
                    if pred == true_label:
                        correct += 1
                print(f"[{done_this_run}/{len(todo)}] {author} / {title[:20]} -> {pred} ({conf})")
            f.flush()
